...
class KorgeKingdomMainScene(
	...
	suspend override fun sceneInit(sceneView: Container) {
		val tileset = TileSet(views, avatarTexture, 32, 32)
		...
		roomNameText = root.text("Room", font = font)
			...
			when (it) {
				...
				is EntityPackets.Server.Set -> {
					entities.getOrPut(it.id) { map.container() }.apply {
						...
						text(it.name, textSize = 22.0, font = font) {
							x = 10.0
							y = 10.0
						}
						x = it.pos.x
						...
						if (time > 0.0) {
							queue {
								view.tween(
										view::x[packet.pos.x],
										view::y[packet.pos.y],
										time = time.milliseconds, easing = Easing.EASE_IN_OUT_QUAD
								)
							}
						}
					}
